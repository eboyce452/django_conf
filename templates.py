    ...
    def handle(self, app_or_project, name, target=None, **options):
        ...
        if target is None:
            ...
            try:
                ...
                '''
					The code that I added in below auto-creates a lot of the templates and files that basically every project uses and for some reason you had to manually create
					To start, it creates directories for templates and static. Then, inside the static directory it creates three more directories: css, js, and sitepackages
					The css and js folders have a commented string written to a file named 'project' + 'css/js' that just says the type of file it is with instructions for resetting browser cache
					The sitepackages directory is populated with bootstrap.min.css, bootstrap.min.js, popper.min.js, and jquery.min.js by creating a dictionary then os.walking for the files and shutil copying them
                '''
                ...
                if self.app_or_project == 'project':
                    ...
                    try:
                        ...
                        sitepackage_path = path.join(static_path, 'sitepackages')

                        os.mkdir(css_path)
                        os.mkdir(js_path)
                        os.mkdir(sitepackage_path)
                    ...
                    for x in package_pathtargs:
                        for root, dirs, files in os.walk(str(pathlib.Path.home())):
                            for sp_utility in files:
                                if sp_utility == x:
                                    package_filepath = os.path.abspath(os.path.join(root, sp_utility))
                                    package_pathtargs[x]=package_filepath

                    PACKAGE_PATHS = [a for a in package_pathtargs.values()]
                    PACKAGE_NAMES = [a for a in package_pathtargs.keys()]
